...
void csi_motion_task(void *unused)
{
    ...
    while (true) {
        ...
        if (now_us - s_last_heal_check_us > 30000000LL) {   /* 30s 节流 */
            ...
            autoheal_check(now_us, config_get_csi_auto_heal());
        }
...
{
    csi_motion_set_enabled(config_get_csi_enabled());
    /* threshold=0 是"自动"默认：开机跳过（避免无谓重校准）；用户显式
     * 从锁定改回 0 时由 config 写路径调 set_threshold(0) 走恢复语义 */
    const float thr = config_get_csi_threshold();
    if (thr > 0.0f) csi_motion_set_threshold(thr);
    csi_motion_set_motion_hits(config_get_csi_on_hits(), config_get_csi_off_hits());
    if (config_get_csi_profile() != s_profile) csi_motion_set_profile(config_get_csi_profile());
}
