...
/* SNTP 时间同步（issue #7）：移植自 seeed-esp32s3-cam main/time_sync.c。
 * 本板此前无任何时间源——ONVIF 事件 UtcTime / GetSystemDateAndTime 一直
 * 是 1970 纪元。系统时间被 SNTP 设置后，onvif_events.c / onvif_service.c
 * 里所有 time(NULL) 派生的时戳自动生效，无需改动。 */
...
esp_err_t time_sync_init(void)
{
    ...
    /* Apply configured timezone before SNTP sync（契约 §3.1：空 = UTC） */
 ...
 * @param tz 时区字符串（POSIX格式），NULL/空时默认 CST-8
 */
