...
RtspServer::RtspServer(const Config &config)
    ...
    , max_sessions_(config.max_sessions) {}
...
size_t RtspServer::get_num_sessions() const {
...
bool RtspServer::accept_task_function(std::mutex &m, std::condition_variable &cv,
  ...
  // MiBee Cam patch: enforce concurrent session cap to bound memory/socket usage.
  if (max_sessions_ > 0) {
    size_t current = 0;
    {
      std::lock_guard<std::mutex> lk(session_mutex_);
    ...
    if (current >= max_sessions_) {
      logger_.warn("Rejecting connection: {} sessions already active (max {})",
                   current, max_sessions_);
      // control_socket is a local unique_ptr; dropping it closes the connection.
      return false;
    }
  }

  // create a new session
  // MiBee patch (2026-09-04): RtspSession ctor starts a std::thread; when the
  // system is out of tasks/heap it throws std::system_error which — uncaught —
  // terminated the whole firmware (abort → rst:0xc). Reject the connection
  // instead of crashing the board.
  std::unique_ptr<RtspSession> session;
  try {
    session = std::make_unique<RtspSession>(
        ...
        RtspSession::Config{.server_address = fmt::format("{}:{}", server_address_, port_),
                            ...
                            .auth_username = auth_username_,
                            .auth_password = auth_password_,
                            .auth_realm = auth_realm_,
                            .log_level = session_log_level_});
  } catch (const std::exception &e) {
    logger_.error("RTSP session creation failed ({}) — rejecting connection", e.what());
    return false;
  }
  if (session->is_closed()) {
