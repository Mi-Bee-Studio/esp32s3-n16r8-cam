...
void Task::thread_function() {
  ...
  while (started_) {
    ...
    // PATCHED (MiBee, 2026-09-05): esp_task_wdt_reset() removed.
    // n16r8 saw a ~150Hz "task not found" UART flood that wedged the board;
    // link-level analysis showed this was the only esp_task_wdt_reset call
    // site in the firmware. Nobody in this project calls start_watchdog(),
    // so the reset could only ever fire spuriously for an unregistered task.
    // Our tasks that need watchdog protection register/unregister explicitly.
#endif // ESP_PLATFORM
  }    // while (started_)
}
